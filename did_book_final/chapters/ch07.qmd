...
| HAD event-study | `did_had` (SSC) + `nprobust` (GitHub) + `gtools` (SSC) | `DIDHAD` (GitHub) + `nprobust` (CRAN) | Not available |
...
df <- as.data.frame(read_dta("pierce_schott_didtextbook.dta"))
...
df = pd.read_stata("pierce_schott_didtextbook.dta")
cmap = plt.cm.Blues
...
**Cross-platform note:** The Stata version of `did_had` (v. 05.08.2025) computes a separate optimal bandwidth for each event-study effect via `lprobust`, while the R version (`DIDHAD` 2.0.0) uses a single bandwidth across all effects. This leads to differences in point estimates. Both implementations give the same quasi-stayer test statistic ($T = 6.1499$, $p = 0.1399$) and qualitatively identical conclusions. The results shown in the PDF textbook ($h^*_G = 0.147$, $N = 21$) match the R output.
...
-------------+---------------------------------------------------------------------------------------------------
    Effect_1 | -.0375279   .0902184  -.1911976   .1624520        103   .1154996         11   6.149888   .1398623 
    Effect_2 | -.2536751   .1789953  -.7355104  -.0338618        103   .0940192          9   6.149888   .1398623 
    Effect_3 |  .0701249   .5633524  -1.433308   .7749930        103   .1688616         22   6.149888   .1398623 
    Effect_4 |  .0256076   .5919349  -1.763432   .5569105        103   .1627251         22   6.149888   .1398623 
----------------------------------------------------------------------------------------------------------------
...
-------------+-----------------------------------------------------------------------------
   Placebo_1 | -.0240233   .1220233  -.3353964   .1429263        103   .1680702         22 
   Placebo_2 | -.0536926   .2453933  -.4238379   .5380862        103   .1550189         21 
   Placebo_3 | -.0694788   .2668673  -.6043572   .4417436        103   .1044157         10 
------------------------------------------------------------------------------------------
```
...
`did_had` is not available in Python.
