...
Note: The Python did-had package (v0.1.0) does not yet handle gaps in
the time series (year 2003 missing), so Effect_3 and Placebo_3 are not
computed. Bandwidth selection also differs from Stata (global vs.
effect-specific). Results will be updated when the package is fixed.
```
