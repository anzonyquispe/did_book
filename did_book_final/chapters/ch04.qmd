...
fml = "patents ~ " + " + ".join(minus_vars + plus_vars) + " | year_x_pat1900 + treatmentgroup"
m = pf.feols(fml, data=df, vcov={"CRV1": "subclass"})
c = m.coef(); s = m.se(); t = m.tstat(); p = m.pvalue()
print(f"{'Variable':<16} {'Coef':>12} {'Std.Err':>12} {'t':>10} {'P>|t|':>10}")
for name in c.index:
    print(f"{name:<16} {c[name]:>12.7f} {s[name]:>12.7f} {t[name]:>10.4f} {p[name]:>10.6f}")
...
print(f"\nF-test: F({len(pre_names)}, {df['subclass'].nunique()-1}) = {F_stat:.6f}, p = {p_val:.6e}")
```
...
delta_rm = hd.createSensitivityResults_relativeMagnitudes(
    ...
    numPrePeriods=18, numPostPeriods=21, Mbarvec=[0.5, 1, 1.5, 2])
print(delta_rm)
```
