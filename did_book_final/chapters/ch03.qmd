...
First, construct the event-study dummies. The reference period is 1918 (the last pre-treatment year). For treated subclasses, `reltimeminus_l` equals 1 at $l$ years before 1918, and `reltimeplus_l` equals 1 at $l$ years after 1918.
...
```stata
* Generate event-study dummies (reference period: 1918)
forvalues l = 1/18 {
...
# Generate event-study dummies (reference period: 1918)
for (l in 1:18) {
    df[[paste0("reltimeminus", l)]] <- as.numeric(df$treatmentgroup == 1 & df$year == 1918 - l)
}
for (l in 1:21) {
    df[[paste0("reltimeplus", l)]] <- as.numeric(df$treatmentgroup == 1 & df$year == 1918 + l)
}
```
