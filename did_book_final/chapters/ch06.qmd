...
```r
library(haven); library(panelView)
df <- read_dta("wolfers2006_didtextbook.dta")
...
df = pd.read_stata("wolfers2006_didtextbook.dta")
...
use "wolfers2006_didtextbook.dta", clear
did_multiplegt_dyn div_rate state year udl, effects(13) placebo(13) ///
...
did_imputation div_rate state year cohort [aweight=stpop], ///
    horizons(0/12) autosample minn(0) pre(13)
```
...
use "wolfers2006_didtextbook.dta", clear
did2s div_rate, first_stage(i.state i.year) ///
    second_stage(rel_timeminus9 rel_timeminus8 rel_timeminus7 ///
    rel_timeminus6 rel_timeminus5 rel_timeminus4 rel_timeminus3 ///
    rel_timeminus2 rel_time1 rel_time2 rel_time3 rel_time4 ///
    rel_time5 rel_time6 rel_time7 rel_time8 rel_time9 rel_time10 ///
    rel_time11 rel_time12 rel_time13 rel_time14 rel_time15 ///
    rel_time16) treatment(udl) cluster(state)
```
...
import pandas as pd
df = pd.read_stata("wolfers2006_didtextbook.dta")
# did2s is not available as a Python package.
# The two-stage estimator (Gardner 2022) can be implemented manually:
# Stage 1: Estimate unit + time FEs on untreated observations (udl == 0).
# Stage 2: Residualize all observations, regress on relative-time dummies.
# See Stata/R tabs for results.
```

```
did2s is not available in Python. See Stata/R tabs for full results.
Coefficients from Stata (rel_timeminus1 omitted, baseline):
  Pre-treatment: small and insignificant (joint test supports parallel trends)
  Post-treatment: negative effects, growing in magnitude from -0.287 (ℓ=1) to -0.822 (ℓ=15)
  Pattern consistent with other heterogeneity-robust estimators (GQ6–GQ9)
```

:::

**Interpretation:** The Gardner (2022) two-stage DID estimates follow the same qualitative pattern as the other heterogeneity-robust estimators: pre-treatment effects are small and insignificant, while post-treatment effects are mostly negative and growing in magnitude. The point estimates from -8 to +15 match exactly between Stata and R (to 7 decimals). These results are broadly consistent with the Sun & Abraham, Callaway & Sant'Anna, and Borusyak et al. estimates, providing additional robustness evidence.
...
```stata
use "wolfers2006_didtextbook.dta", clear
graph combine fig62 g1 g2 g4, cols(2) ///
    title("Effects of Unilateral Divorce Laws") ///
    note("95% CIs shown in red. Weighted by state population. SEs clustered at state level.")
```
