...
```python
# No bacondecomp package available in Python.
# See Stata/R output for the Bacon decomposition.
```
...
```stata
eventstudyweights rel_time1-rel_time16 rel_timeminus1-rel_timeminus9 ///
    [aweight=stpop], absorb(i.state i.year) cohort(cohort) ///
    control_cohort(controlgroup) saveweights(Ws)
```
...
```r
# No direct R equivalent of eventstudyweights.
# See Stata tab or use twowayfeweights (GQ5 below) for decomposition.
```

### Python

```python
# No Python equivalent of eventstudyweights.
# See Stata tab or use twowayfeweights (GQ5 below) for decomposition.
```
