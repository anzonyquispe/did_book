...
```python
# Decomposition (manual implementation of twowayfeweights feTR)
eps = np.array(pf.feols("numdailies ~ 1 | cnty90 + year", data=df).resid())
...
The sum of the negative weights is equal to -0.4740.
```
...
display "numdailies:  coef = " %12.7f _b[numdailies] ///
   "  se = " %12.7f _se[numdailies]
```
...
eps2 = np.array(pf.feols("numdailies ~ C(styr) | cnty90 + year",
                          data=df, fixef_rm="none").resid())
D_vals2 = df['numdailies'].astype(float).values
N2 = len(D_vals2)
denom_W2 = np.mean(eps2 * D_vals2)
weight2 = eps2 * D_vals2 / (N2 * denom_W2)
```
...
mean_D0 = work['D0'].mean()
mean_wD0 = (work['w'] * work['D0']).mean()
work['sigma'] = work['w'] * mean_D0 / mean_wD0

sub = att & np.isfinite(work['sigma'].values)
reg_df = pd.DataFrame({
    'year_val': work.loc[sub, 'year_orig'].values,
    'sigma': work.loc[sub, 'sigma'].values,
    'nat_w': work.loc[sub, 'D0'].values,
    'G': work.loc[sub, 'G'].astype(str).values
})
reg_df['nat_w'] = reg_df['nat_w'] / reg_df['nat_w'].sum()

rw_reg = pf.feols("year_val ~ sigma", data=reg_df,
                   weights="nat_w", vcov={"CRV1": "G"})
```

```
The coefficient of year in the regression of the weights on year is:
    Coef          SE         t       Corr(weights, year)
  -0.1674271   0.0510117   -3.2821306       -0.0631614
```
