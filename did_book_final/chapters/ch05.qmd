...
Dataset: **Gentzkow, Shapiro & Sinkinson (2011)** — `gentzkowetal_didtextbook.dta (Stata), gentzkowetal_didtextbook.RData (R), gentzkowetal_didtextbook.parquet (Python)`

This chapter investigates what the TWFE estimator $\hat{\beta}^{fe}$ estimates outside the classical design, using the effect of newspapers on voter turnout. The design features a non-binary treatment (number of newspapers), non-absorbing treatment changes, and variation in treatment timing across 1,195 US counties from 1868 to 1928.
...
## CQ#18–19: Basic TWFE Regression & Weight Decomposition
...
result = twowayfeweights(df, "prestout", "cnty90", "year", "numdailies",
                          type="feTR")
print_twowayfeweights(result, D_name="numdailies", type="feTR")
```
...
## CQ#20–21: TWFE Regression with State-Year FEs & Weight Decomposition
...
styr_dummies <- model.matrix(~ factor(styr) - 1, data = df)
colnames(styr_dummies) <- paste0("styr", 1:ncol(styr_dummies))
df <- cbind(df, styr_dummies)
styr_cols <- paste0("styr", 1:ncol(styr_dummies))
...
# Generate state-year dummies (not pre-computed in the parquet)
styr_dummies = pd.get_dummies(df["styr"], prefix="styr", prefix_sep="", dtype=int)
styr_cols = list(styr_dummies.columns)
df = pd.concat([df, styr_dummies], axis=1)
result = twowayfeweights(df, "prestout", "cnty90", "year", "numdailies",
...
## CQ#22–24: FD Regression, Weight Decomposition & Random Weights Test
...
result3 = twowayfeweights(df, "changeprestout", "cnty90", "year",
                           ...
                           controls=styr_cols)
print_twowayfeweights(result3, D_name="changedailies", type="fdTR")
```
...
print_twowayfeweights(result3b, D_name="changedailies", type="fdTR")
```
