...
panelview prestout numdailies, i(cnty90) t(year) type(treat) ///
    title("Gentzkow et al. (2011)") legend(off) ///
    ylabel(none) ytitle("")
...
df = pd.read_stata("gentzkowetal_didtextbook.dta")
...
```r
library(haven); library(lmtest); library(sandwich)
df <- read_dta("gentzkowetal_didtextbook.dta")
...
gq4.fit(); gq4.summary()
mat = gq4.result['mat_res_XX']
mat = mat[mat.index != 4].copy()
ref = pd.DataFrame({'Estimate': [0], 'LB CI': [0], 'UB CI': [0], 'Time': [0]})
mat = pd.concat([mat, ref], ignore_index=True).sort_values('Time')
fig, ax = plt.subplots(figsize=(8, 5))
ax.plot(mat['Time'], mat['Estimate'], 'o-', color='navy', markersize=5, linewidth=1.5)
ax.errorbar(mat['Time'], mat['Estimate'],
    yerr=[mat['Estimate']-mat['LB CI'], mat['UB CI']-mat['Estimate']],
    fmt='none', ecolor='firebrick', capsize=3, linewidth=1.2)
ax.axhline(0, color='black', linewidth=0.8)
ax.axvline(0, color='black', linewidth=0.8)
ax.set_xlabel("Relative time to last period before treatment changes (t=0)")
ax.set_title("DID, from last period before treatment changes (t=0) to t")
plt.tight_layout()
...
df <- as.data.frame(read_dta("gentzkowetal_didtextbook.dta"))
...
df_pl = pl.from_pandas(df)
pd.set_option('display.float_format', lambda x: f'{x:.7f}')
# design() option not yet available in py-did-multiplegt-dyn.
for ell in [1, 2, 4]:
...
--- effects = 1 ---
             Estimate        SE      LB CI     UB CI      N  Switchers
Effect_1     0.0144244 0.0042477  0.0060992 0.0227497   5674      1119
Av_tot_eff   0.0120186 0.0035392  0.0050819 0.0189552   5674      1119
...
Joint nullity p-value = 0.001482
...
Joint nullity p-value = 0.006814

All effects match Stata/R to 7 decimal places.
Treatment path tables not available (design option not in Python package).
```
...
----------------------------------------------------------------------
     Estimate    SE      LB CI   UB CI   Switchers Stayers
AOSS 0.0062363 0.0014311 0.0034312 0.0090413 4,423     11,043

----------------------------------------------------------------------
               Estimation of AOSS(s) - Placebo
----------------------------------------------------------------------
      Estimate    SE      LB CI    UB CI   Switchers Stayers
AOSS -0.0009239 0.0014709 -0.0038067 0.0019590 2,287     7,743

----------------------------------------------------------------------
                    Estimation of WAOSS(s)
----------------------------------------------------------------------
      Estimate    SE      LB CI   UB CI   Switchers Stayers
WAOSS 0.0059026 0.0013034 0.0033480 0.0084572 4,423     11,043

----------------------------------------------------------------------
               Estimation of WAOSS(s) - Placebo
----------------------------------------------------------------------
      Estimate    SE      LB CI    UB CI   Switchers Stayers
WAOSS 0.0002176 0.0013382 -0.0024053 0.0028404 2,287     7,743
...
----------------------------------------------------------------------
       Estimate         SE      LB CI      UB CI Switchers Stayers
AOSS  0.0061034  0.0016286  0.0029113  0.0092954     4,423  11,043
...
----------------------------------------------------------------------
        Estimate         SE      LB CI      UB CI Switchers Stayers
WAOSS  0.0057527  0.0014895  0.0028333  0.0086721     4,423  11,043

----------------------------------------------------------------------
               Estimation of WAOSS(s) - Placebo(s)
----------------------------------------------------------------------
                   Estimate         SE       LB CI      UB CI Switchers Stayers
Placebo_1_waoss  -0.0000125  0.0022931  -0.0045070  0.0044820     2,287   7,743
```

:::

**Interpretation:** Both ATS ($\approx 0.006$) and WATS ($\approx 0.006$) are significant and positive across all three platforms. The placebo estimates are small and insignificant, supporting the parallel-trends assumption. These estimates are roughly half the magnitude of $\widehat{AVSQ}^n_1 = 0.012$, which suggests a non-linear treatment response: the average effect per unit of treatment change is smaller when computed across all switcher magnitudes.
