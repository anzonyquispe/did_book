...
Dataset: **Gentzkow, Shapiro & Sinkinson (2011)** — `gentzkowetal_didtextbook.dta (Stata), gentzkowetal_didtextbook.RData (R), gentzkowetal_didtextbook.parquet (Python)` (16,872 observations, 1,195 counties, 16 election years 1868–1928)
...
7 computer questions + 2 Figures + 1 Bonus test.
...
**Note on cross-platform differences:** CQ40–CQ43 and CQ45 match exactly between Stata and R to 7 decimals. CQ49 (`did_multiplegt_stat`) shows small differences between Stata, R, and Python due to internal implementation variations.. N, switchers, and stayers counts are identical across all three.

------------------------------------------------------------------------

## CQ#40: Distributed-Lag TWFE Regression

> Regress turnout on the number of newspapers and its lag, with county and year FEs, clustering standard errors at the county level. Interpret the results.

::: panel-tabset
### Stata

``` stata
copy "https://raw.githubusercontent.com/Credible-Answers/did_book/main/cc_xd_didtextbook_2025_9_30/Data%20sets/Gentzkow%20et%20al%202011/gentzkowetal_didtextbook.dta" "gentzkowetal_didtextbook.dta", replace
...
load(url("https://raw.githubusercontent.com/Credible-Answers/did_book/main/cc_xd_didtextbook_2025_9_30/Data%20sets/Gentzkow%20et%20al%202011/gentzkowetal_didtextbook.RData"))
gq3 <- feols(prestout ~ numdailies + lag_numdailies | cnty90 + year, data = df, cluster = ~cnty90)
summary(gq3)
```
...
df = pd.read_parquet("https://raw.githubusercontent.com/Credible-Answers/did_book/main/cc_xd_didtextbook_2025_9_30/Data%20sets/Gentzkow%20et%20al%202011/gentzkowetal_didtextbook.parquet")
m8 = pf.feols("prestout ~ numdailies + lag_numdailies + C(year) | cnty90", data=df, vcov={"CRV1": "cnty90"}, ssc=pf.ssc(adj=True, fixef_k="full", cluster_adj=True))
print(m8.summary())
```
...
**Interpretation:** The distributed-lag TWFE regression gives $\hat\beta_0^{dl} = -0.0007962$ (insignificant) and $\hat\beta_1^{dl} = 0.0050348$ (significant). The contemporaneous effect of newspapers on turnout is small and insignificant, while the lagged effect is positive and significant.

------------------------------------------------------------------------

## CQ#41: Weight Decomposition (`twowayfeweights`)

> Use the `twowayfeweights` command and the `other_treatments` option to decompose $\hat\beta_0^{dl}$ and $\hat\beta_1^{dl}$, and interpret the results.
...
load(url("https://raw.githubusercontent.com/Credible-Answers/did_book/main/cc_xd_didtextbook_2025_9_30/Data%20sets/Gentzkow%20et%20al%202011/gentzkowetal_didtextbook.RData"))
gq3a <- twowayfeweights(df, "prestout", "cnty90", "year", "numdailies", type = "feTR", other_treatments = "lag_numdailies")
gq3b <- twowayfeweights(df, "prestout", "cnty90", "year", "lag_numdailies", type = "feTR", other_treatments = "numdailies")
```
...
df = pd.read_parquet("https://raw.githubusercontent.com/Credible-Answers/did_book/main/cc_xd_didtextbook_2025_9_30/Data%20sets/Gentzkow%20et%20al%202011/gentzkowetal_didtextbook.parquet")
res_a = twowayfeweights(df, "prestout", "cnty90", "year", "numdailies", type="feTR", other_treatments="lag_numdailies")
res_b = twowayfeweights(df, "prestout", "cnty90", "year", "lag_numdailies", type="feTR", other_treatments="numdailies")
```

```         
Decomposition of beta on numdailies:
Positive weights: 5754, Σ = 1.8541
Negative weights: 4302, Σ = -0.8541
Other treat (lag_numdailies): 4721 pos (1.2137) | 4618 neg (-1.2137)

Decomposition of beta on lag_numdailies:
Positive weights: 5273, Σ = 1.7820
Negative weights: 4066, Σ = -0.7820
Other treat (numdailies): 4933 pos (1.3558) | 5123 neg (-1.3558)
```
:::

**Interpretation:** Both coefficients are severely contaminated: the positive weights sum to 1.85, the negative weights to $-0.85$, and the cross-contamination from the other treatment sums to $\pm 1.21$. This means neither coefficient can be interpreted as a convex combination of causal effects.

------------------------------------------------------------------------

## CQ#42: Non-Normalized Event-Study Effects (Figure 8.1)

> Use `did_multiplegt_dyn` to compute, for $\ell \in \{1, \ldots, 4\}$, non-normalized event-study estimates $\widehat{AVSQ}_\ell$ of the effect of being exposed to a weakly larger number of newspapers for $\ell$ election cycles on turnout, as well as a test that effects are all equal using the `effects_equal` option. Also compute the pre-trend estimates $\widehat{AVSQ}_{-\ell}$ for $\ell \in \{1, \ldots, 4\}$.
...
copy "https://raw.githubusercontent.com/Credible-Answers/did_book/main/cc_xd_didtextbook_2025_9_30/Data%20sets/Gentzkow%20et%20al%202011/gentzkowetal_didtextbook.dta" "gentzkowetal_didtextbook.dta", replace
use "gentzkowetal_didtextbook.dta", clear
did_multiplegt_dyn prestout cnty90 year numdailies, effects(4) placebo(4) effects_equal(all) cluster(cnty90)
...
----------------------------------------------------------------------
 Estimate        SE     LB CI     UB CI         N Switchers
  0.0160565   0.0047761   0.0066956   0.0254174     8,659     4,074
...
----------------------------------------------------------------------
             Estimate SE      LB CI    UB CI   N     Switchers
Placebo_1   -0.0005025 0.0051322 -0.0105615 0.0095565 4,471   902
Placebo_2    0.0020594 0.0085031 -0.0146063 0.0187251 2,778   746
Placebo_3   -0.0015365 0.0116825 -0.0244337 0.0213607 1,644   604
...
gq4 <- did_multiplegt_dyn(
    ...
    effects = 4, placebo = 4, effects_equal = TRUE, cluster = "cnty90")
...
Test of joint nullity of the effects : p-value = 0.0068
Test of equality of the effects : p-value = 0.4152
Av_tot_eff = 0.01606 (SE = 0.00478)

Placebo_1   -0.00050 0.00513  4,471   902
Placebo_2    0.00206 0.00850  2,778   746
Placebo_3   -0.00154 0.01168  1,644   604
Placebo_4    0.00066 0.01750    910   441
...
gq4 = DidMultiplegtDyn(df=df_pl, outcome="prestout", group="cnty90",
    time="year", treatment="numdailies", effects=4, placebo=4, effects_equal=True, cluster="cnty90")
gq4.fit(); gq4.summary()
gq4.plot()
...
**Interpretation:** All four non-normalized event-study effects are positive and significant, suggesting newspapers increase turnout. The effects grow with $\ell$, from 0.0144 at $\ell = 1$ to 0.0273 at $\ell = 4$. However, one cannot reject the null that all effects are equal (p-value = 0.4152). All placebo estimates are small and insignificant, with a joint p-value of 0.9922, strongly supporting the parallel-trends assumption. The average cumulative effect per treatment unit is 0.0161 (SE = 0.0048).
...
## CQ#43–44: Treatment Path Descriptions
...
--------------------------------------------------------------------------------
Average number of time periods over which a treatment's effect is accumulated = 1
...
--------------------------------------------------------------------------------
             ...
             |   #Groups    %Groups        ℓ=0        ℓ=1 
-------------+-------------------------------------------
  TreatPath1 |       721      64.03          0          1 
  TreatPath2 |       139      12.34          0          2 
  TreatPath3 |        54      4.796          1          2 
--------------------------------------------------------------------------------
 Treatment paths detected in at least 80.00% of the switching groups 1126 for which 1 effects could be estimated.
 Total % = 81.17 %
 Design interpretation (first row):
 721 groups started with treatment 0 and then experienced treatment 1.
```
...
--------------------------------------------------------------------------------
 Treatment paths detected in at least 80.00% of the switching groups 1067 for which 2 effects could be estimated.
 Total % = 81.54 %
 Design interpretation (first row):
 343 groups started with treatment 0 and then experienced treatment vector (1, 1).
```
...
--------------------------------------------------------------------------------
Average number of time periods over which a treatment's effect is accumulated = 2.4375615
...
--------------------------------------------------------------------------------
             ...
             |   #Groups    %Groups        ℓ=0        ℓ=1        ℓ=2        ℓ=3        ℓ=4 
-------------+----------------------------------------------------------------------------
  TreatPath1 |       141      15.11          0          1          1          1          1 
  TreatPath2 |       127      13.61          0          1          0          0          0 
  TreatPath3 |        43      4.609          0          1          2          2          2 
  TreatPath4 |        40      4.287          0          1          1          1          2 
  TreatPath5 |        32       3.43          0          1          1          2          2 
  TreatPath6 |        19      2.036          0          1          2          1          1 
  TreatPath7 |        18      1.929          0          1          1          1          0 
  TreatPath8 |        18      1.929          0          2          1          1          1 
  TreatPath9 |        16      1.715          0          1          1          0          0 
 TreatPath10 |        16      1.715          0          2          2          2          2 
 TreatPath11 |        15      1.608          0          1          0          0          1 
 TreatPath12 |        15      1.608          0          1          1          2          1 
 TreatPath13 |        15      1.608          0          1          2          2          1 
 TreatPath14 |        12      1.286          0          1          0          1          1 
 TreatPath15 |        12      1.286          0          2          1          2          2 
 TreatPath16 |        11      1.179          0          1          2          1          2 
 TreatPath17 |        10      1.072          1          2          2          2          2 
 TreatPath18 |         8      .8574          0          1          2          2          3 
 TreatPath19 |         8      .8574          0          2          2          1          1 
 TreatPath20 |         8      .8574          1          0          0          0          0 
 TreatPath21 |         7      .7503          0          1          1          0          1 
 TreatPath22 |         7      .7503          0          1          1          3          3 
 TreatPath23 |         7      .7503          0          1          2          3          2 
 TreatPath24 |         7      .7503          0          1          2          3          3 
 TreatPath25 |         7      .7503          0          1          3          2          2 
 TreatPath26 |         6      .6431          0          1          0          1          2 
 TreatPath27 |         6      .6431          0          1          1          2          3 
 TreatPath28 |         6      .6431          0          2          0          0          0 
 TreatPath29 |         6      .6431          0          2          2          3          2 
 TreatPath30 |         6      .6431          1          2          2          2          1 
 TreatPath31 |         6      .6431          2          1          1          1          1 
 TreatPath32 |         5      .5359          0          1          3          3          3 
 TreatPath33 |         5      .5359          0          2          1          1          2 
 TreatPath34 |         5      .5359          0          2          2          1          2 
 TreatPath35 |         5      .5359          0          2          2          2          3 
 TreatPath36 |         5      .5359          0          2          3          2          2 
 TreatPath37 |         5      .5359          2          3          2          2          2 
 TreatPath38 |         4      .4287          0          1          1          1          3 
 TreatPath39 |         4      .4287          0          1          3          2          3 
 TreatPath40 |         4      .4287          0          2          2          2          1 
 TreatPath41 |         4      .4287          0          2          2          3          3 
 TreatPath42 |         4      .4287          0          3          2          3          2 
 TreatPath43 |         4      .4287          1          2          2          3          3 
 TreatPath44 |         3      .3215          0          1          0          0          2 
 TreatPath45 |         3      .3215          0          1          0          1          0 
 TreatPath46 |         3      .3215          0          1          1          0          2 
 TreatPath47 |         3      .3215          0          1          1          2          4 
 TreatPath48 |         3      .3215          0          1          1          3          2 
 TreatPath49 |         3      .3215          0          1          3          3          2 
 TreatPath50 |         3      .3215          0          2          1          2          1 
 TreatPath51 |         3      .3215          0          2          3          4          3 
 TreatPath52 |         3      .3215          1          0          0          0          1 
 TreatPath53 |         3      .3215          1          2          1          2          2 
 TreatPath54 |         3      .3215          1          2          2          1          1 
 TreatPath55 |         3      .3215          1          2          2          2          3 
 TreatPath56 |         3      .3215          2          1          1          2          2 
--------------------------------------------------------------------------------
 Treatment paths detected in at least 80.00% of the switching groups 933 for which 4 effects could be estimated.
 Total % = 80.17 %
 Design interpretation (first row):
 141 groups started with treatment 0 and then experienced treatment vector (1, 1, 1, 1).
```
...
load(url("https://raw.githubusercontent.com/Credible-Answers/did_book/main/cc_xd_didtextbook_2025_9_30/Data%20sets/Gentzkow%20et%20al%202011/gentzkowetal_didtextbook.RData")); df <- as.data.frame(df)
...
TreatPath3       54  4.7957371 2.0000000 3.0000000
TreatPath4       54  4.7957371 1.0000000 2.0000000
================================================================================
Treatment paths detected in switching groups: 1126
Total % shown: 85.97%

Design interpretation (first row):
  721 groups started with treatment 0.0 and then experienced treatment [np.float64(1.0)]
================================================================================
             Estimation of treatment effects: Event-study effects
================================================================================
               Block   Estimate        SE      LB CI     UB CI            N    Switchers          N.w  Switchers.w
            Effect_1  0.0144244 0.0042477  0.0060992 0.0227497 5674.0000000 1119.0000000 5674.0000000 1119.0000000
Average_Total_Effect  0.0120186 0.0035392  0.0050819 0.0189552 5674.0000000 1119.0000000 5674.0000000 1119.0000000
           Placebo_1 -0.0005025 0.0051322 -0.0105615 0.0095565 4471.0000000  902.0000000 4471.0000000  902.0000000
================================================================================
...
TreatPath10       18  1.6869728 2.0000000 3.0000000 3.0000000
================================================================================
Treatment paths detected in switching groups: 1067
Total % shown: 83.22%

Design interpretation (first row):
  343 groups started with treatment 0.0 and then experienced treatment [np.float64(1.0), np.float64(1.0)]
================================================================================
             Estimation of treatment effects: Event-study effects
================================================================================
               Block   Estimate        SE      LB CI     UB CI            N    Switchers          N.w  Switchers.w
            Effect_1  0.0144244 0.0042477  0.0060992 0.0227497 5674.0000000 1119.0000000 5674.0000000 1119.0000000
            Effect_2  0.0190899 0.0058429  0.0076381 0.0305418 4648.0000000 1054.0000000 4648.0000000 1054.0000000
Average_Total_Effect  0.0143270 0.0039890  0.0065087 0.0221454 6754.0000000 2173.0000000 6754.0000000 2173.0000000
           Placebo_1 -0.0005025 0.0051322 -0.0105615 0.0095565 4471.0000000  902.0000000 4471.0000000  902.0000000
================================================================================
...
TreatPath4        40  4.2872454 0.0000000 1.0000000 1.0000000 1.0000000 2.0000000
TreatPath5        32  3.4297964 0.0000000 1.0000000 1.0000000 2.0000000 2.0000000
TreatPath6        19  2.0364416 0.0000000 1.0000000 2.0000000 1.0000000 1.0000000
TreatPath7        18  1.9292605 0.0000000 2.0000000 1.0000000 1.0000000 1.0000000
TreatPath8        18  1.9292605 0.0000000 1.0000000 1.0000000 1.0000000 0.0000000
TreatPath9        16  1.7148982 0.0000000 1.0000000 1.0000000 0.0000000 0.0000000
TreatPath10       16  1.7148982 0.0000000 2.0000000 2.0000000 2.0000000 2.0000000
TreatPath11       15  1.6077170 0.0000000 1.0000000 0.0000000 0.0000000 1.0000000
TreatPath12       15  1.6077170 0.0000000 1.0000000 1.0000000 2.0000000 1.0000000
TreatPath13       15  1.6077170 0.0000000 1.0000000 2.0000000 2.0000000 1.0000000
TreatPath14       12  1.2861736 0.0000000 1.0000000 0.0000000 1.0000000 1.0000000
TreatPath15       12  1.2861736 0.0000000 2.0000000 1.0000000 2.0000000 2.0000000
TreatPath16       11  1.1789925 0.0000000 1.0000000 2.0000000 1.0000000 2.0000000
TreatPath17       10  1.0718114 1.0000000 2.0000000 2.0000000 2.0000000 2.0000000
TreatPath18        8  0.8574491 0.0000000 2.0000000 2.0000000 1.0000000 1.0000000
TreatPath19        8  0.8574491 1.0000000 0.0000000 0.0000000 0.0000000 0.0000000
TreatPath20        8  0.8574491 0.0000000 1.0000000 2.0000000 2.0000000 3.0000000
TreatPath21        7  0.7502680 0.0000000 1.0000000 2.0000000 3.0000000 2.0000000
TreatPath22        7  0.7502680 0.0000000 1.0000000 1.0000000 0.0000000 1.0000000
TreatPath23        7  0.7502680 0.0000000 1.0000000 3.0000000 2.0000000 2.0000000
TreatPath24        7  0.7502680 0.0000000 1.0000000 1.0000000 3.0000000 3.0000000
TreatPath25        7  0.7502680 0.0000000 1.0000000 2.0000000 3.0000000 3.0000000
TreatPath26        6  0.6430868 1.0000000 2.0000000 2.0000000 2.0000000 1.0000000
TreatPath27        6  0.6430868 0.0000000 2.0000000 2.0000000 3.0000000 2.0000000
TreatPath28        6  0.6430868 0.0000000 2.0000000 0.0000000 0.0000000 0.0000000
TreatPath29        6  0.6430868 0.0000000 1.0000000 0.0000000 1.0000000 2.0000000
TreatPath30        6  0.6430868 0.0000000 1.0000000 1.0000000 2.0000000 3.0000000
TreatPath31        6  0.6430868 2.0000000 1.0000000 1.0000000 1.0000000 1.0000000
TreatPath32        5  0.5359057 0.0000000 2.0000000 2.0000000 1.0000000 2.0000000
TreatPath33        5  0.5359057 0.0000000 2.0000000 3.0000000 2.0000000 2.0000000
TreatPath34        5  0.5359057 0.0000000 1.0000000 3.0000000 3.0000000 3.0000000
TreatPath35        5  0.5359057 0.0000000 2.0000000 2.0000000 2.0000000 3.0000000
TreatPath36        5  0.5359057 2.0000000 3.0000000 2.0000000 2.0000000 2.0000000
TreatPath37        5  0.5359057 0.0000000 2.0000000 1.0000000 1.0000000 2.0000000
TreatPath38        4  0.4287245 0.0000000 3.0000000 2.0000000 3.0000000 2.0000000
TreatPath39        4  0.4287245 0.0000000 2.0000000 2.0000000 3.0000000 3.0000000
TreatPath40        4  0.4287245 1.0000000 2.0000000 2.0000000 3.0000000 3.0000000
TreatPath41        4  0.4287245 0.0000000 1.0000000 3.0000000 2.0000000 3.0000000
TreatPath42        4  0.4287245 0.0000000 1.0000000 1.0000000 1.0000000 3.0000000
TreatPath43        4  0.4287245 0.0000000 2.0000000 2.0000000 2.0000000 1.0000000
TreatPath44        3  0.3215434 2.0000000 1.0000000 1.0000000 2.0000000 2.0000000
TreatPath45        3  0.3215434 1.0000000 2.0000000 2.0000000 1.0000000 1.0000000
TreatPath46        3  0.3215434 2.0000000 3.0000000 2.0000000 2.0000000 4.0000000
TreatPath47        3  0.3215434 2.0000000 3.0000000 3.0000000 3.0000000 3.0000000
TreatPath48        3  0.3215434 0.0000000 1.0000000 1.0000000 0.0000000 2.0000000
TreatPath49        3  0.3215434 2.0000000 3.0000000 2.0000000 3.0000000 2.0000000
TreatPath50        3  0.3215434 0.0000000 1.0000000 1.0000000 3.0000000 2.0000000
TreatPath51        3  0.3215434 0.0000000 1.0000000 0.0000000 0.0000000 2.0000000
TreatPath52        3  0.3215434 2.0000000 3.0000000 3.0000000 4.0000000 3.0000000
TreatPath53        3  0.3215434 0.0000000 1.0000000 0.0000000 1.0000000 0.0000000
TreatPath54        3  0.3215434 1.0000000 2.0000000 1.0000000 2.0000000 2.0000000
TreatPath55        3  0.3215434 2.0000000 1.0000000 2.0000000 2.0000000 2.0000000
TreatPath56        3  0.3215434 0.0000000 1.0000000 1.0000000 2.0000000 4.0000000
TreatPath57        3  0.3215434 2.0000000 3.0000000 3.0000000 3.0000000 2.0000000
================================================================================
Treatment paths detected in switching groups: 933
Total % shown: 80.49%

Design interpretation (first row):
  141 groups started with treatment 0.0 and then experienced treatment [np.float64(1.0), np.float64(1.0), np.float64(1.0), np.float64(1.0)]
================================================================================
             Estimation of treatment effects: Event-study effects
================================================================================
               Block   Estimate        SE      LB CI     UB CI            N    Switchers          N.w  Switchers.w
            Effect_1  0.0144244 0.0042477  0.0060992 0.0227497 5674.0000000 1119.0000000 5674.0000000 1119.0000000
            Effect_2  0.0190899 0.0058429  0.0076381 0.0305418 4648.0000000 1054.0000000 4648.0000000 1054.0000000
            Effect_3  0.0207147 0.0079164  0.0051989 0.0362305 3750.0000000  984.0000000 3750.0000000  984.0000000
            Effect_4  0.0272653 0.0097924  0.0080727 0.0464580 2980.0000000  917.0000000 2980.0000000  917.0000000
Average_Total_Effect  0.0160565 0.0047761  0.0066956 0.0254174 8659.0000000 4074.0000000 8659.0000000 4074.0000000
           Placebo_1 -0.0005025 0.0051322 -0.0105615 0.0095565 4471.0000000  902.0000000 4471.0000000  902.0000000
================================================================================
Test of joint nullity of the effects: p-value = 0.006814
...
## CQ#45: Path-Specific Event-Study Effects (`by_path`)

> Estimate the first two event-study effects separately for the three most common $(D_{g,F_g-1}, D_{g,F_g}, D_{g,F_g+1})$ paths shared by more than 100 counties, using `by_path(3)`.
...
copy "https://raw.githubusercontent.com/Credible-Answers/did_book/main/cc_xd_didtextbook_2025_9_30/Data%20sets/Gentzkow%20et%20al%202011/gentzkowetal_didtextbook.dta" "gentzkowetal_didtextbook.dta", replace
use "gentzkowetal_didtextbook.dta", clear
did_multiplegt_dyn prestout cnty90 year numdailies, effects(2) by_path(3)
```

```         
--- Path (0, 1, 1) ---
             |  Estimate         SE      LB CI      UB CI          N  Switchers
    Effect_1 |  .0131318   .0071358  -.0008542   .0271178       4529        343
    Effect_2 |   .009357   .0090553   -.008391    .027105       3713        343
Test of joint nullity of the effects : p-value = .1750087

--- Path (0, 1, 0) ---
             |  Estimate         SE      LB CI      UB CI          N  Switchers
    Effect_1 |  .0144984   .0103889  -.0058635   .0348603       4373        187
    Effect_2 |  .0082605   .0115639  -.0144043   .0309254       3557        187
Test of joint nullity of the effects : p-value = .32227545

--- Path (0, 1, 2) ---
             |  Estimate         SE      LB CI      UB CI          N  Switchers
    Effect_1 |  .0212045   .0103034   .0010102   .0413988       4317        131
    Effect_2 |  .0314473   .0132732   .0054324   .0574622       3501        131
Test of joint nullity of the effects : p-value = .05711112
```
...
library(haven); library(DIDmultiplegtDYN)
load(url("https://raw.githubusercontent.com/Credible-Answers/did_book/main/cc_xd_didtextbook_2025_9_30/Data%20sets/Gentzkow%20et%20al%202011/gentzkowetal_didtextbook.RData"))
did_multiplegt_dyn(df = as.data.frame(df), outcome = "prestout", group = "cnty90", time = "year", treatment = "numdailies", effects = 2, by_path = 3)
```
...
df = pd.read_parquet("https://raw.githubusercontent.com/Credible-Answers/did_book/main/cc_xd_didtextbook_2025_9_30/Data%20sets/Gentzkow%20et%20al%202011/gentzkowetal_didtextbook.parquet")
df_pl = pl.from_pandas(df)
bp = DidMultiplegtDyn(df=df_pl, outcome="prestout", group="cnty90", time="year", treatment="numdailies", effects=2, by_path=3, cluster="cnty90")
bp.fit(); bp.summary()
```

```         
Note: The Python py_did_multiplegt_dyn package groups treatment paths differently
from Stata/R (e.g., (None, 1, None) instead of (0, 1, 1)). Results are not
directly comparable. Use Stata or R for by_path analysis.
```
:::

**Interpretation:** Event-study effects are insignificant for the $(D_{g,F_g-1}, D_{g,F_g}, D_{g,F_g+1}) = (0, 1, 1)$ and $(0, 1, 0)$ paths, and significant for the $(0, 1, 2)$ path. This might suggest that sustained exposure to an increasing number of newspapers is necessary to generate an effect, though event-study effects are noisy and may not significantly differ across paths.

------------------------------------------------------------------------

## CQ#46–47: Normalized Event-Study Effects (Figure 8.2)
...
copy "https://raw.githubusercontent.com/Credible-Answers/did_book/main/cc_xd_didtextbook_2025_9_30/Data%20sets/Gentzkow%20et%20al%202011/gentzkowetal_didtextbook.dta" "gentzkowetal_didtextbook.dta", replace
use "gentzkowetal_didtextbook.dta", clear
did_multiplegt_dyn prestout cnty90 year numdailies, effects(4) placebo(4) normalized effects_equal(all) normalized_weights cluster(cnty90)
...
----------------------------------------------------------------------
             Estimate SE      LB CI   UB CI   N     Switchers
Effect_1     0.0120186 0.0035392 0.0050819 0.0189552 5,674 1,119
Effect_2     0.0082126 0.0025136 0.0032859 0.0131392 4,648 1,054
Effect_3     0.0057192 0.0021857 0.0014354 0.0100030 3,750   984
...
----------------------------------------------------------------------
             Estimate SE      LB CI    UB CI   N     Switchers
Placebo_1   -0.0004158 0.0042470 -0.0087399 0.0079082 4,471   902
Placebo_2    0.0008699 0.0035919 -0.0061701 0.0079099 2,778   746
Placebo_3   -0.0004201 0.0031943 -0.0066808 0.0058406 1,644   604
Placebo_4    0.0001394 0.0037128 -0.0071375 0.0074164   910   441
...
gq6 <- did_multiplegt_dyn(
    ...
    time = "year", treatment = "numdailies",
    effects = 4, placebo = 4,
    normalized = TRUE, normalized_weights = TRUE, effects_equal = TRUE, cluster = "cnty90")
...
## CQ#48: Joint Test of No Dynamic Effects
...
================================================================================
               Block  Estimate       SE     LB CI    UB CI      N  Switchers    N.w  Switchers.w
            Effect_1  0.015250 0.005896  0.003695 0.026805 5019.0      512.0 5019.0        512.0
            Effect_2  0.016409 0.007363  0.001977 0.030841 4101.0      512.0 4101.0        512.0
Average_Total_Effect  0.013831 0.005328  0.003388 0.024273 5605.0     1024.0 5605.0       1024.0
           Placebo_1 -0.001460 0.006378 -0.013961 0.011041 3936.0      413.0 3936.0        413.0
================================================================================
...
## CQ#49: ATS and WATS Estimators (`did_multiplegt_stat`)
...
| CQ# | Method | Key Finding |
|:---:|:---:|:---|
| CQ#40 | Distributed-lag TWFE | Current effect NS (−0.0008), lag significant (0.0050); severe negative weights (Σ = −0.85) and cross-contamination (Σ = ±1.21) |
| CQ#42 | `did_multiplegt_dyn` | All effects positive & significant; placebos NS (joint p = 0.992) |
| CQ#43–44 | `design` option | Treatment paths increasingly heterogeneous as ℓ grows |
| CQ#46–47 | Normalized ES | Decreasing effects; cannot reject equality (p = 0.165); roughly equal lag weights |
| CQ#48 | No-dynamics test | Effects 1 & 2 not different (p = 0.83) → consistent with no dynamic effects |
| CQ#49 | `did_multiplegt_stat` | ATS ≈ WATS ≈ 0.006 (significant); placebos NS |